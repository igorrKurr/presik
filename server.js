...
const FLAGS_WITH_VALUE = new Set(['dir', 'port', 'key', 'qr', 'db', 'group']);
const positional = [];
...
const groupSlug = (g) => (g ? g.trim().replace(/[^\p{L}\p{N}_-]+/gu, '-').slice(0, 40) : null);
...
const MARP_BIN = path.join(ENGINE_DIR, 'node_modules', '.bin', 'marp');
...
function ensureSlideHtml() {
  ...
  if (stale) {
    execFileSync(MARP_BIN, [DECK_FILE, '-o', DECK_HTML, '--html', '--allow-local-files'], { stdio: 'inherit' });
...
// ---------------------------------------------------------------- state
const state = { index: -1, revealed: false };
...
// Live in-class state stays in answers (the Map above); this is the
// long-term archive for analysis via report.js.
const db = openDb(DB_FILE);
let runId = null;

function startRun() {
  const info = db
    .prepare('INSERT INTO runs (course, session, group_name, title, started_at) VALUES (?, ?, ?, ?, ?)')
    .run(course, name, GROUP || null, quiz.title || name, new Date().toISOString());
...
function broadcast() {
  for (const s of streams) {
    try {
      s.res.write('data: ' + snapshot(s.host) + '\n\n');
    } catch (_) {}
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (p === '/api/answer' && req.method === 'POST') {
    ...
    const cid = String(clientId).slice(0, 64);
    const val = String(value).slice(0, 600);
  ...
  if (p === '/api/control' && req.method === 'POST') {
    const { action, key, to } = await readBody(req);
...
function localIp() {
  ...
  const n = all.find((i) => i && i.family === 'IPv4' && !i.internal);
  return n ? n.address : 'localhost';
}
